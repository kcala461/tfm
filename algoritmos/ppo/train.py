...
class SaveActionsRewardsCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.num_timesteps % self.save_freq == 0:
            # Guardar el modelo
            model_save_path = f"{self.save_model_path}.zip" #_{self.num_timesteps}
...
save_path = "kepajo.txt"
save_model_path = "model/CurvaMultip2"
save_freq = 200000  # Frecuencia para guardar el modelo
...
model.load("model/CurvaMultip")

model.learn(total_timesteps=2100000, callback=save_callback, log_interval=100000)
